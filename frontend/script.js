// ===== 1. API KEY (Safe: browser local storage) =====
let API_KEY = localStorage.getItem('jarvis_key');
if (!API_KEY) {
  API_KEY = prompt('AQ.Ab8RN6KqzCPoqMFTUftWJ7OULGAermjOgFKC3eJBdbzVPa3mCw:');
  if (API_KEY) localStorage.setItem('jarvis_key', API_KEY);
}

// ===== 2. SMART MODELS (auto fallback) =====
...
async function askGemini(p) {
  ...
  try {
    ...
    speak(reply); // Speech output on reply
  } catch (e) {
...
if (SR) {
  const rec = new SR();
  rec.lang = 'en-US'; // Change to 'te-IN' for Telugu
